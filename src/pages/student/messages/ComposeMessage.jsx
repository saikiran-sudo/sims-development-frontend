...
function ComposeMessage({ onClose, onSend, onSaveDraft, replyTo }) {
  ...
  const [validUserIds, setValidUserIds] = useState(new Set());

  // Helper function to check if a user ID is valid
  const isValidUserId = useCallback((userId) => {
    return validUserIds.has(userId) || 
           Array.from(validUserIds).some(id => 
             id.toLowerCase() === userId.toLowerCase()
           );
  }, [validUserIds]);

  // Helper function to check if input looks like a user ID
  const looksLikeUserId = useCallback((input) => {
    return /[A-Za-z].*\d|\d.*[A-Za-z]/.test(input);
  }, []);
  ...
  // Load group counts and available user IDs on component mount
  useEffect(() => {
    fetchGroupCounts();
    
    // Fetch available user IDs and valid user IDs for validation
    const fetchUserData = async () => {
      try {
        ...
        setValidUserIds(new Set(userIds));
        
        // Also set initial recipient options
        ...
        console.log('Setting initial recipient options:', options);
        setIndividualRecipientOptions(options);
      } catch (error) {
  ...
  const handleRecipientsChange = async (selectedOptions, actionMeta) => {
    if (actionMeta.action === 'create-option') {
      ...
      // Check if the input looks like a user ID
      if (looksLikeUserId(newInputValue)) {
        // If it looks like a user ID, check if it exists in our valid user IDs
        if (!isValidUserId(newInputValue)) {
          console.error(`Invalid User ID: "${newInputValue}". User ID not found in system.`);
          
          // Provide helpful suggestions
          const suggestions = availableUserIds
            .filter(id => id.toLowerCase().includes(newInputValue.toLowerCase()))
            .slice(0, 3);
          
          let errorMessage = `User ID "${newInputValue}" not found.`;
          if (suggestions.length > 0) {
            errorMessage += ` Did you mean: ${suggestions.join(', ')}?`;
          } else {
            errorMessage += ` Please check the ID or select from suggestions.`;
          }
      ...
      if (foundUser) {
        ...
        console.error(`User ID not found: "${newInputValue}". Please check the ID or select from suggestions.`);
  ...
  const handleSubmit = async (asDraft = false) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/under-my-admin-for-student`, formDataToSend, {
        headers: {
          ...getAuthHeaders(),
          ...
          {messageType === 'individual' ? (
            <div>
              ...
              <CreatableSelect
                ...
                placeholder="Type to search users by name, email, or user ID..."
                ...
                isValidNewOption={(inputValue, selectValue, selectOptions) => {
                  ...
                  if (looksLikeUserId(inputValue)) {
                    // If it looks like a user ID, check if it exists in our valid user IDs
                    return isValidUserId(inputValue);
                  }
                  
                  // Allow other inputs (names, emails) to be processed normally
                  return true;
                }}
                isLoading={isLoadingOptions}
                noOptionsMessage={() => "No users found. Type to search..."}
              ...
              <p className="mt-1 text-xs text-gray-500">
                💡 You can search by name, email, or user ID. User IDs are validated against the system.
                {availableUserIds.length > 0 && (
                  <span className="block mt-1">
                    Available IDs: {availableUserIds.slice(0, 5).join(', ')}{availableUserIds.length > 5 ? '...' : ''}
                  </span>
                )}
                <span className="block mt-1 text-blue-600">
                  Debug: {individualRecipientOptions.length} options loaded
                </span>
              </p>
            </div>
          ) : (
