...
function AnnouncementDetails({ data, editable = false, onClose }) { // Remove onUpdate prop
  ...
  React.useEffect(() => {
    if (data) {
      setFormData({
        title: data.title,
        content: data.content,
        target: (data.targetGroups || []).map(t => targetOptions.find(option => option.value === t)),
  ...
  const handleSubmit = async() => {
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/api/announcements/${data._id}`, updatedAnnouncement,{ // Using API_BASE_URL
          ...
          {!editable && (
            <div>
              ...
              <div className="p-2 bg-gray-100 rounded text-sm">
                {new Date(data.createdAt).toLocaleString()} {/* Use data.createdAt directly */}
              </div>
            </div>
          )}
        </div>
