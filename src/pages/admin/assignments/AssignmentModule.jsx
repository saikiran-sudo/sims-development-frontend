...
const EditAssignment = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`${API_BASE_URL}/api/assignments/${editedAssignment._id}`, payload, { // Use API_BASE_URL
        headers: {
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          {/* Class */}
          {console.log('editedAssignment.class is ', allAvailableClasses)}
          <div>
            ...
            <select
              id="edit-class"
              name="class"
              value={typeof editedAssignment.class === 'object' ? editedAssignment.class.class_name : editedAssignment.class.class_name}
              onChange={handleInputChange}
          ...
          {/* Due Date */}
          {console.log('editedAssignment.dueDate is ', editedAssignment.dueDate ? new Date(editedAssignment.dueDate).toLocaleDateString() : '')}
          <div className="sm:col-span-2">
            ...
            <input
              ...
              id="edit-dueDate"
              name="dueDate"
              value={editedAssignment.dueDate ? new Date(editedAssignment.dueDate).toISOString().split('T')[0]: ''}
              onChange={handleInputChange}
...
const AssignmentModule = () => {
  ...
  const [allAvailableSections, setAllAvailableSections] = useState(SECTION_OPTIONS);
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/api/subjects`, { // Use API_BASE_URL
          headers: {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      ...
      try {
        ...
        // Also update the existing state variables for backward compatibility
        setAllAvailableClasses(uniqueClasses);
        setAllAvailableSections(uniqueSections);
      } catch (error) {
        ...
        setAllAvailableSections(SECTION_OPTIONS);
      } finally {
  ...
  useEffect(() => {
    const fetchAssignmentsWithSubmissionCounts = async () => {
      ...
      try {
        ...
        const assignmentsRes = await axios.get(`${API_BASE_URL}/api/assignments`, { // Use API_BASE_URL
          headers: token ? { Authorization: `Bearer ${token}` } : {}
        });
        ...
        const assignmentsWithCounts = await Promise.all(
          assignmentsData.map(async (assignment) => {
            try {
              const submissionsRes = await axios.get(`${API_BASE_URL}/api/assignment-submissions/${assignment._id}`, { // Use API_BASE_URL
                headers: token ? { Authorization: `Bearer ${token}` } : {}
              });
  ...
  const fetchSubmissionsForAssignment = async (assignmentId) => {
    try {
      const res = await axios.get(`${API_BASE_URL}/assignment-submissions/${assignmentId}`, getAuthHeaders()); // Use API_BASE_URL
  ...
  const handleCreateAssignment = async (assignmentData) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/api/assignments/`, assignmentData, getAuthHeaders()); // Use API_BASE_URL
  ...
  const handleUpdateAssignment = async (id, updatedData) => {
    try {
      const res = await axios.put(`${API_BASE_URL}/api/assignments/${id}`, updatedData, getAuthHeaders()); // Use API_BASE_URL
  ...
  const deleteAssignment = async (id) => {
    if (window.confirm('Are you sure you want to delete this assignment?')) {
      try {
        await axios.delete(`${API_BASE_URL}/api/assignments/${id}`, getAuthHeaders()); // Use API_BASE_URL
  ...
  const [assignmentState, setAssignmentState] = useState(initialAssignmentsWithCounts);
  ...
  const [selectedClassFilterState, setSelectedClassFilterState] = useState("All Classes");
  ...
  const filteredAssignmentsState = assignments.filter(assignment => {
    const classMatch = selectedClassFilterState === "All Classes" || assignment.class?.class_name === selectedClassFilterState;
  ...
  const totalStudentsForFilter = students.filter(student => {
    ...
    const classMatch = selectedClassFilterState === "All Classes" || className === selectedClassFilterState;
    const sectionMatch = selectedSectionFilterState === "All" || section === selectedSectionFilterState;
    return classMatch && sectionMatch;
  }).length;

  const handleBackToAssignments = (classFilterFromSubmissions = "All Classes") => {
  ...
  if (showViewSubmissionsPageState) {
    return (
      <ViewSubmissions
        ...
        allAvailableClasses={classOptions} // Pass available classes for dropdown
      />
    );
  }
  ...
  return (
    <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
            ...
            {filteredAssignmentsState.length > 0 ? (
              filteredAssignmentsState.map((assignment) => (
                <tr key={assignment._id} className="transition-colors duration-200 hover:bg-indigo-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{assignment.subject?.name || assignment.subject}</td>
      ...
      <CreateAssignment
        ...
        allAvailableClasses={classOptions.filter(cls => cls !== "All Classes")} // Pass filtered classes
        allAvailableSections={sectionOptions} // Pass sections
        allAvailableSubjects={allAvailableSubjects} // Pass subjects to CreateAssignment
      />
      ...
      <EditAssignment
        ...
        assignments={assignments}
        setAssignments={setAssignments}
        allAvailableClasses={classOptions.filter(cls => cls !== "All Classes")}
        allAvailableSections={sectionOptions}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">{selectedAssignmentState.subject?.name || selectedAssignmentState.subject}</p>
                  </div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 flex items-center gap-3">
