...
const ViewSubmissions = ({ onBackToAssignmentModule, selectedClassFilter, allAvailableClasses }) => {
      ...
      try {
        ...
        const submissionsArr = await Promise.all(
          assignmentsRes.data.map(async (assignment) => {
            try {
              const res = await axios.get(`${API_BASE_URL}/api/assignment-submissions/${assignment._id}`, { // Use API_BASE_URL
                headers: token ? { Authorization: `Bearer ${token}` } : {}
              });
              // Each submission has files: []
              console.log('API response for submissions:', res.data);
  ...
  // Handle saving the grade from the modal (PATCH to backend)
  const saveGrade = async () => {
    if (!selectedSubmission) return;
    try {
      const token = JSON.parse(localStorage.getItem('authToken'));
      await axios.put(`${API_BASE_URL}/api/assignment-submissions/grade/${selectedSubmission.submissionId}`, { // Use API_BASE_URL
        grade: currentGrade,
