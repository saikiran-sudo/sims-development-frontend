...
function MessageModule() {
  ...
  const filteredMessages = useMemo(() => {
    ...
    return messages.filter(message => {
      ...
      // Filter by tab
      if (activeTab === 'inbox') {
        console.log('Inbox filter - message status:', message.status, 'sender:', message.sender);
        // For inbox, show messages where user is a recipient and status is not draft/trash
        if (message.status === 'draft' || message.status === 'trash') {
          console.log('Excluding message - draft or trash status');
          return false; // Exclude drafts and trash
        }
      }

      if (activeTab === 'sent') {
        console.log('Sent filter - checking if user is sender');
        // For sent, show messages where user is the sender and status is sent
        if (message.status !== 'sent') {
          console.log('Sent filter - excluding non-sent message');
          return false;
        }
      }
      
      if (activeTab === 'drafts' && message.status !== 'draft') {
        console.log('Drafts filter - excluding message');
        return false;
      }

      // Trash logic: only show messages with status 'trash' and within the retention period
      if (activeTab === 'trash') {
        if (message.status !== 'trash') {
          console.log('Trash filter - excluding non-trash message');
          return false;
        }
        if (message.deletedAt) {
          const days = getDaysSinceDeletion(message.deletedAt);
          if (days > TRASH_RETENTION_DAYS) {
            console.log('Trash filter - excluding expired message');
            return false; // Hide if past retention
          }
        } else {
          // Fallback: if status is trash but no deletedAt, assume it's current trash
          // In a real app, deletedAt should always be set.
        }
      }

      // Starred logic: show messages that are starred AND NOT in trash
      if (activeTab === 'starred' && (!message.starred || message.status === 'trash')) {
        console.log('Starred filter - excluding message');
        return false;
      }
      ...
      if (filters.search) {
        ...
        if (!matches) {
          console.log('Search filter - excluding message');
          return false;
        }
      }
      ...
      if (filters.status) {
        if (filters.status.value === 'read' && !message.read) {
          console.log('Status filter - excluding unread message');
          return false;
        }
        if (filters.status.value === 'unread' && message.read) {
          console.log('Status filter - excluding read message');
          return false;
        }
        if (filters.status.value === 'sent' && message.status !== 'sent') {
          console.log('Status filter - excluding non-sent message');
          return false;
        }
        if (filters.status.value === 'draft' && message.status !== 'draft') {
          console.log('Status filter - excluding non-draft message');
          return false;
        }
      }

      // Apply date range filter (simplified for example, would need more complex logic for actual date comparison)
      if (filters.dateRange) {
        ...
        if (messageDate < startDate || messageDate >= endDate) {
          console.log('Date filter - excluding message');
          return false;
        }
      }
  ...
  const unreadCount = useMemo(() => {
    const count = messages.filter(m => !m.read && m.status === 'sent' && m.status !== 'trash').length;
    console.log('Unread count calculation:', count, 'Total messages:', messages.length);
    console.log('Messages for unread count:', messages.filter(m => !m.read && m.status === 'sent' && m.status !== 'trash'));
    return count;
  }, [messages]);
    ...
    <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
      ...
      <div className="border-b border-gray-200 mb-6">
        <nav className="-mb-px flex space-x-4 md:space-x-8 justify-around md:justify-start"> {/* Adjusted spacing for mobile */}
          <button
            onClick={() => { setActiveTab('inbox'); setExpandedMessage(null); }}
          ...
          <button
            onClick={() => { setActiveTab('sent'); setExpandedMessage(null); }}
          ...
          <button
            onClick={() => { setActiveTab('drafts'); setExpandedMessage(null); }}
          ...
          <button
            onClick={() => { setActiveTab('starred'); setExpandedMessage(null); }}
          ...
          <button
            onClick={() => { setActiveTab('trash'); setExpandedMessage(null); }}
          ...
          {loading ? (
            <div className="px-6 py-12 text-center bg-white">
              <div className='flex flex-col items-center justify-center text-gray-500'>
                ...
                <h3 className='text-lg font-semibold mb-1'>Loading messages...</h3>
                <p className='text-sm mt-1 text-gray-600'>Please wait while we fetch your messages.</p>
              </div>
            </div>
          ) : error ? (
            <div className="px-6 py-12 text-center bg-white">
              <div className='flex flex-col items-center justify-center text-gray-500'>
                <h3 className='text-lg font-semibold mb-1 text-red-600'>Error loading messages</h3>
                            ...
                            {activeTab === 'drafts' && (
                                <button
                                    onClick={(e) => {
                                        ...
                                        setExpandedMessage(null); // Collapse the message after sending
                                    }}
              ...
              <div className='flex flex-col items-center justify-center text-gray-500'>
                ...
                <p className='text-sm mt-1 text-gray-600'>
                  {messages.length === 0 ?
                    ...
                    `No messages found in ${activeTab}. Try clearing your filters or selecting a different tab.`}
                </p>
                {messages.length === 0 && (
                  <button
                    onClick={() => setShowComposeModal(true)}
      ...
      {showComposeModal && (
        <ComposeMessage
          onClose={() => {
            ...
            setSelectedMessage(null); // Clear selected message on close
          }}
