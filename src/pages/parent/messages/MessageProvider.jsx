...
export const MessageProvider = ({ children }) => {
  ...
  const fetchMessages = useCallback(async (tab = activeTab, filterOverrides = {}) => {
    ...
    try {
      ...
      console.log('Making API call to:', `${API_BASE_URL}/under-my-parent?${params.toString()}`);
      console.log('Auth headers:', getAuthHeaders());
      console.log('Current activeTab:', activeTab);
      console.log('Tab parameter:', tab);
      
      const res = await axios.get(`${API_BASE_URL}/under-my-parent?${params.toString()}`, {
      ...
      console.log('First message sample:', messageArray[0]);
  ...
  useEffect(() => {
    console.log('MessageProvider useEffect triggered - activeTab:', activeTab, 'filters:', filters);
    fetchMessages();
  ...
  const handleSendMessage = async (messageData) => {
    ...
    try {
      ...
      // await axios.post(`${API_BASE_URL}/under-my-admin-for-parent`, messageData, {
      //   headers: {
      //     ...getAuthHeaders(),
  ...
  const handleSaveDraft = async (messageData) => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/under-my-admin-for-parent`, formData, {
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'multipart/form-data',
        },
  ...
  const handleDeleteMessage = async (id) => {
    ...
    setError(null);
    try {
      await axios.patch(`${API_BASE_URL}/${id}/delete-under-my-admin-for-parent`, {}, { headers: getAuthHeaders() });
      await fetchMessages();
    } catch (err) {
      setError(err.response?.data?.message || err.message);
    } finally {
  ...
  const handleUndoDelete = async (id) => {
    ...
    setError(null);
    try {
      await axios.patch(`${API_BASE_URL}/${id}/undo-under-my-admin-for-parent`, {}, { headers: getAuthHeaders() });
      await fetchMessages();
    } catch (err) {
      setError(err.response?.data?.message || err.message);
    } finally {
  ...
  const handleMarkAsRead = async (id) => {
    ...
    setError(null);
    try {
      await axios.put(`${API_BASE_URL}/${id}/read-under-my-admin-for-parent`, {}, { headers: getAuthHeaders() });
      await fetchMessages();
    } catch (err) {
      setError(err.response?.data?.message || err.message);
    } finally {
