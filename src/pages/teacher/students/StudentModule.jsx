...
const StudentModule = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      setError(null);
      try {
        // Use the new endpoint to get only students assigned to the teacher's class
        const res = await api.get('/students/by-class-teacher');
        ...
        const mapped = (res.data || []).map(s => ({
          ...
          parentDisplay: Array.isArray(s.parent_id) && s.parent_id.length > 0 ? s.parent_id.map(p => `${p.full_name} (${p.user_id})`).filter(Boolean).join(', ') : '',
          status: s.status,
        ...
        // If teacher is not assigned as class teacher, show appropriate message
        if (err.response?.status === 404 && err.response?.data?.message?.includes('not assigned as a class teacher')) {
          setError('You are not assigned as a class teacher. Please contact your administrator.');
        } else {
  ...
  useEffect(() => {
    const fetchParents = async () => {
      ...
      try {
        const res = await parentAPI.getAllParentsUnderMyAdmin();
        setParentOptions(
          (res.data || []).map(p => ({
    ...
    <div className="px-0 sm:px-2 md:px-4 lg:p-6 flex flex-col gap-2 sm:gap-4 lg:gap-8">
          ...
          <div className="p-4 sm:p-6"> {/* Reduced padding for smaller screens */}
            ...
            <div className="mt-2">
              <div className="flex items-center space-x-2 mb-1">
                ...
                <span className="text-xs font-medium text-gray-500">My Class Students</span>
              </div>
      ...
      {/* Main Content */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">My Class Students</h2>
            <p className="text-sm text-gray-600 mt-1">Students assigned to your class</p>
          </div>
                ...
                <tr>
                  <td colSpan="10" className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">
                    <p className="text-center text-gray-500 py-4">
                      {students.length === 0 
                        ? "No students are currently assigned to your class. Please contact your administrator if you believe this is an error."
