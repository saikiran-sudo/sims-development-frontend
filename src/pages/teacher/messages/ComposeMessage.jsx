...
function ComposeMessage({ onClose, onSend, onSaveDraft, replyTo }) {
  ...
  const fetchGroupCounts = useCallback(async () => {
    ...
    try {
      const counts = {};
      for (const group of ['all_students', 'all_teachers', 'all_parents', 'all_admins', 'all']) {
  ...
  const handleSubmit = async (asDraft = false) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/under-my-admin`, formDataToSend, {
        headers: {
          ...getAuthHeaders(),
          ...
          {messageType === 'individual' ? (
            <div>
              ...
              <CreatableSelect
                ...
                placeholder="Type to search users by name, email, or user ID (e.g., S001, T001, P001, A001)..."
              ...
              <p className="mt-1 text-xs text-gray-500">
                💡 You can search by name, email, or user ID. User IDs follow the format: S001 (Student), T001 (Teacher), P001 (Parent), A001 (Admin)
              </p>
            </div>
          ) : (
            <div>
