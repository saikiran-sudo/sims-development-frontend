...
import { ChevronLeft, ChevronRight, CheckCircle, XCircle, Clock, AlertCircle, Ban } from 'lucide-react';
...
const Calendar = ({ currentMonth, monthlyAttendanceData, selectedDate, setSelectedDate, setCurrentMonth }) => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case 'late':
                // Changed icon to AlertCircle for 'late' for consistency and clarity
                return <AlertCircle className="text-yellow-500" size={18} />; 
            case 'half-day':
                return <Clock className="text-blue-500" size={18} />;
            case 'leave':
                return <Ban className="text-orange-600" size={18} />;
            default:
                return null;
        }
    };
    ...
    const generateCalendarDays = () => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            // Do NOT set attendance to null for Sundays here. Let the rendering logic handle it.
            days.push({
                date,
                dateStr,
                attendance 
                ...
                {days.map((day, index) => (
                    <div
                        ...
                        className={`bg-white min-h-16 p-1 relative flex flex-col justify-between items-center text-center
                            ...
                            ${day && day.attendance && day.date.getDay() !== 0 ? 'bg-indigo-50' : ''} {/* Apply bg-indigo-50 ONLY if attendance exists AND it's not a Sunday */}
